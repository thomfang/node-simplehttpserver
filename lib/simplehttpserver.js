...
function SimpleHTTPServer(options) {
...
SimpleHTTPServer.prototype = {
    ...
    _requestHandler: function (request, response) {
        ...
        if (realpath.code === 301) {
            response.writeHead(301, { 'Location': path.join(realpath.path, '/') });
            response.end('301 ');
            ...
            var ctnType = mime.lookup(realpath) || 'text/plain';
            var self = this;
    ...
    _checkRange: function (pathname, ext, st, request, response) {
        ...
        if (request.headers['range']) {
            ...
            if (range) {
                response.setHeader('Content-Range', 'bytes ' + range.start + '-' + range.end + '/' + st.size);
                response.setHeader('Content-Length', range.end - range.start + 1);
                
                var opts = { start: range.start, end: range.end };

                raw = fs.createReadStream(pathname, opts);
